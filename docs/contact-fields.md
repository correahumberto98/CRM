...
Drag a contact card to a different stage; the destination is outlined in blue. Dropping saves a stage-only PATCH through the current authenticated session, then refreshes the board and counts. The existing backend records the actor, old/new stage and timestamp in contact history and resets stage entry time. Same-stage drops do nothing. Only one move is submitted at a time; failed saves show an error and keep the original board. Each card also has a Stage selector for keyboard/touch access. No stage is a legacy source column, not a drop destination, because stage is required. Existing filters and pagination continue to apply after moves.
