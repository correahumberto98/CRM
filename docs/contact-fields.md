...
Column dragging uses a rectangular preview of the header and visible cell values with a drop shadow. The source column is dimmed, and a blue vertical insertion line spans the destination column header and cells. Crossing a header midpoint moves the insertion target before/after that column; dropping uses that boundary and saves the order. Approaching the horizontal scroll area edges scrolls the table. Canceling a drag clears the preview and marker without saving a new order.
